...
public class MyBackgroundRemovalManager : MonoBehaviour
{
    ...
    private static MyBackgroundRemovalManager instance;

    private Dictionary<int, Texture> playerForegroundTextures = new Dictionary<int, Texture>();


    public static MyBackgroundRemovalManager Instance
    {
        get
        {
            return instance;
    ...
    void Update()
    {
        if (isBrInited)
        {
            ...
            if (kinectManager && kinectManager.IsInitialized())
            {
                for (int playerIndex = 0; playerIndex < kinectManager.GetUsersCount(); playerIndex++)
                {
